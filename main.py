# =====================================================================
# SEIZURE MONITOR BACKEND - v7 SD QUEUE FIX
#
# PREVIOUS FIXES (v1):
...
#
# DURATION FIX (v6):
# [FIX I] end_time now uses now_utc (server time) for user sessions
#         start_time uses ts_utc (ESP32 time) for accuracy
# [FIX J] /api/seizure_events/latest prioritizes GTCS over Jerk
# [FIX K] duration_seconds added to responses
#
# SD QUEUE FIX (v7):
# [FIX L] Device session close time uses ts_utc when ts_utc > start_time,
#         falls back to now_utc otherwise.
#         Root cause of fake long durations from SD card bursts:
#         - ESP32 queues only 2 rows offline (flag=true, flag=false)
#         - Backend was using now_utc (WiFi reconnect time) as end_time
#         - start_time was ts_utc (actual event time) → huge fake duration
#         - Fix: use ts_utc as end_time when it's a valid closing timestamp
# [FIX M] STALE_SESSION_THRESHOLD_SECONDS lowered 120 → 45 seconds
#         Shorter window catches orphaned sessions faster after disconnect
# =====================================================================
...
# [FIX A] Raised from 30 to 60 seconds — Render cold-start tolerance
CONNECTED_THRESHOLD_SECONDS = 60

# [FIX M] Lowered from 120 to 45 seconds
# Shorter stale window catches orphaned sessions faster after disconnect
STALE_SESSION_THRESHOLD_SECONDS = 45
...
# =====================================================================
# ESP32 UPLOAD - v7 FIXED
# [FIX L] Device session close time: use ts_utc when ts_utc > start_time
#         (accurate for SD card queued events), else fall back to now_utc
# =====================================================================
...
async def upload_device_data(payload: UnifiedESP32Payload):
    ...
    if payload.seizure_flag:
        ...
        if active_device:
            # [FIX L] Use ts_utc as close time if it's after the session start.
            # This gives accurate duration for SD card queued events:
            #   - Row 1 (flag=true):  start_time = ts_utc of seizure start
            #   - Row 2 (flag=false): end_time   = ts_utc of seizure end
            # Both timestamps come from the ESP32 real-time clock,
            # so the duration = actual seizure length, not WiFi reconnect lag.
            #
            # Fall back to now_utc only if ts_utc is somehow before start_time
            # (e.g. clock drift or invalid timestamp)
            if ts_utc > active_device["start_time"]:
                close_time = ts_utc
                print(f"[SESSION CLOSE] Using ts_utc: {to_pht(ts_utc).strftime('%H:%M:%S')} "
                      f"(start={to_pht(active_device['start_time']).strftime('%H:%M:%S')})")
            else:
                close_time = now_utc
                print(f"[SESSION CLOSE] ts_utc <= start_time, falling back to now_utc: "
                      f"{to_pht(now_utc).strftime('%H:%M:%S')}")

            await database.execute(
                device_seizure_sessions.update()
                .where(device_seizure_sessions.c.id == active_device["id"])
                .values(end_time=close_time)
            )
    ...
    # ── User seizure session logic (duration-based classification) ──
    ...
    # ── No seizure — close open sessions if minimum duration met ──
